                ...
                if ((expiryMonth >= 1 && expiryMonth <= 12) && (expiryYear >= 2017 && expiryYear <= 2051)) {
                    if ((expiryYear == 2017 && expiryMonth > 4) || (expiryYear > 2017)) {
                        if ((cvv >= 1 && cvv <= 9999) && (cvv.length >= 3 && cvv.length <= 4)) {
                            ...
                            $.ajax({
                                ...
                                error: function (xhr, ajaxOptions, thrownError) {
                                    if (xhr.status === 200) {	// receiving 200 status code
                                        ...
                                        $('#error-add-card-message').html('Эта карта уже сохранена')
                                    }
                                },
                            ...
                            $('#success-add-card').addClass('hidden')
                            $('#error-add-card').removeClass('hidden')
                            $('#error-add-card-message').html('Неправильный CVV')
                        }
                    } else {
                        $('#success-add-card').addClass('hidden')
                        $('#error-add-card').removeClass('hidden')
                        $('#error-add-card-message').html('Срок действия карты истек')
                    }
                } else {
                    $('#success-add-card').addClass('hidden')
                    $('#error-add-card').removeClass('hidden')
                    $('#error-add-card-message').html('Неправильный срок истечения')
                }
            } else {
                $('#success-add-card').addClass('hidden')
                $('#error-add-card').removeClass('hidden')
                $('#error-add-card-message').html('Неправильный номер карты')
            }
        } else {
            $('#success-add-card').addClass('hidden')
            $('#error-add-card').removeClass('hidden')
            $('#error-add-card-message').html('Поля не заполнены')
        }
